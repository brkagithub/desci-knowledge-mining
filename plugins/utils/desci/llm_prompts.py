from plugins.utils.desci.examples_for_prompts import (
...
def get_go_api_prompt(term, go_candidates):
    return f"""
    ...
    GO Candidates in JSON format: {str(go_candidates)}
...
def get_doid_api_prompt(term, doid_candidates):
    return f"""
    ...
    DOID Candidates in JSON format: {str(doid_candidates)}
...
def get_chebi_api_prompt(term, chebi_candidates):
    return f"""
    ...
    ChEBI Candidates in JSON format: {str(chebi_candidates)}
...
def get_atc_api_prompt(term, atc_candidates):
    return f"""
    ...
    ATC Candidates in JSON format: {str(atc_candidates)}
...
def get_prompt_basic_info(paper_array):
    return f"""**Prompt**:
...
def get_prompt_citations(paper_array):
    return f"""**Prompt**:
...
def get_prompt_go_subgraph(paper_array):
    return f"""**Prompt**:
...
def get_prompt_doid_subgraph(paper_array):
    return f"""**Prompt**:
...
def get_prompt_chebi_subgraph(paper_array):
    return f"""**Prompt**:
...
def get_prompt_atc_subgraph(paper_array):
    return f"""**Prompt**:
...
def get_prompt_spar_citations(citations):
    return f"""
    **Task**
...
def get_prompt_spar_ontology(basic_info_text):
    return f"""
    ** Task: **
...
def get_prompt_go_ontology(generated_go_subgraph):
    return f"""
    ** Task: **
    ...
    ** Actual Input JSON **
    Gene Ontology terms and relationships: {generated_go_subgraph}
...
def get_prompt_doid_ontology(generated_doid_subgraph):
    return f"""
    ** Task: **
    ...
    ** Actual Input JSON **
    Disease ontology terms and relationships: {generated_doid_subgraph}
...
def get_prompt_chebi_ontology(generated_chebi_subgraph):
    return f"""
    ** Task: **
    ...
    ** Actual Input JSON **
    Disease ontology terms and relationships: {generated_chebi_subgraph}
...
def get_prompt_section_page_numbers(paper_array, sections):
    prompt = f"Given the following pages of a research paper, identify the start and stop pages for each one of the provided sections\n\n"

    for element in paper_array:
        page_number = element["metadata"]["page_number"]
        text = element["text"]
        prompt += f"Page {page_number}:\n{text}\n\n"
    prompt += f"""Please provide the start and stop pages for each section in the following format:
    ...
    ** Your output **
    {[f"{section}, start, end" for section in sections]}
...
def get_prompt_vectorization_summary(graph):
    graph_copy = copy.deepcopy(graph)

    if "cito:cites" in graph_copy:
        del graph_copy["cito:cites"]
    ...
    ** Notes **
    1. Ensure the summary captures all key entities and relationships present in the RDF JSON-LD graph.
    2. The summary should be formatted in a way that makes it easy to use for similarity search purposes - make sure to use specific term names and not pronouns such as "it", "he", "they".
    3. Your output should be only the generated summary. No other comments or remarks will be tolerated.
    ** Actual Input JSON **

    {json.dumps(graph_copy, indent=4)}
    
    ** FINAL NOTE - MAKE SURE TO ONLY OUTPUT THE GENERATED SUMMARY, WITHOUT EXTRA COMMENTS OR REMARKS **
    """


def get_prompt_convert_to_json(incorrect_json):
    return f"""
    ** Task: **
...
def get_prompt_suggested_questions(graph):
    graph_copy = copy.deepcopy(graph)
